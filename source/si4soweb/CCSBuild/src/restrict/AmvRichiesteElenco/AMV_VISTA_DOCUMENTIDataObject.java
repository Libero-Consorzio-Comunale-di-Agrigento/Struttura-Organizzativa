...
//load @5-C2D15CE7
    boolean load() {
        ...
        command.setSql( "SELECT a.ID_DOCUMENTO "
                    ...
                    + "OR ad4_utente.get_nominativo(a.AUTORE) LIKE '%{s_TESTO}%' "
                    + ") "
                    + "" );
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT a.ID_DOCUMENTO , a.TITOLO , b.TITOLO MODELLO , a.DATA_INSERIMENTO ,  "
                                                         ...
                                                         + "            'T','') flusso ,  "
                                                         + "            amv_revisione.get_img_stato(a.STATO,a.TIPO_TESTO) STATO_DOCUMENTO ,  "
                                                         + "            '<a href=\"../restrict/ServletModulisticaView.do?ID='||a.id_documento||'&IDPD='||a.id_documento_padre||'&REV='||a.revisione ||decode( a.stato||amv_revisione.get_diritto_modifica('{Utente}',a.id_documento, a.revisione) , 'B1', '&rw=R' , '&rw=R' ) ||'&'||a.link||'&'||replace(amv_documento.get_modello(a.id_documento_padre,a.revisione),'/','&') ||'\" target=\"_blank\" >'||a.id_documento||'</a>' documento_link FROM AMV_DOCUMENTI a, AMV_DOCUMENTI b WHERE '{Utente}' != 'GUEST' AND a.ID_DOCUMENTO_PADRE = b.ID_DOCUMENTO AND a.REVISIONE = b.REVISIONE and amv_revisione.get_diritto('{Utente}',a.id_documento, a.revisione) is not null AND instr(b.LINK,'iter=N inoltro=I') < 1 AND (decode(nvl({MVID},0),0,a.STATO,'') in ('B','N','V') OR decode(nvl({MVID},0),1,a.STATO,'') in ('A','F')) AND a.id_documento_padre = nvl({s_MODELLO},a.id_documento_padre) AND (a.titolo LIKE '%{s_TESTO}%' OR to_char(a.data_inserimento,'dd/mm/yyyy hh24:mi') LIKE '%{s_TESTO}%' OR amv_revisione.get_des_stato(a.STATO) LIKE '%{s_TESTO}%' OR ad4_utente.get_nominativo(a.AUTORE) LIKE '%{s_TESTO}%' )  ) cnt " );
        if ( ! StringUtils.isEmpty( orderBy ) ) {
            command.setOrder( orderBy );
        } else {
            command.setOrder( "a.DATA_INSERIMENTO" );
        }

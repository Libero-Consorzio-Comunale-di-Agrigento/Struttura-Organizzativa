//AMV_DOCUMENTO_APPROVADataObjectHandler head @6-63336E1B
...
public class AMV_DOCUMENTO_APPROVADataObjectHandler implements RecordDataObjectListener {
//End AMV_DOCUMENTO_APPROVADataObjectHandler head
...
//Event AfterExecuteUpdate Action Custom Code @36-44795B7A
/* -------------------------- *
 *  write your own code here  *
 * -------------------------- */
 String url = (String) e.getRecord().getControl("MVPAGES").getValue();
 url = url.replace('*','&');
 if (url.indexOf("ServletModulistica.do") > 0 ) {
 	url = "../" + url + "?" + e.getPage().getRequest().getQueryString() + "&rw=R";	
 } else if (url.indexOf("AdmDocumento.do") > 0 ) {
	url = "../amvadm/AdmRevisioniElenco.do";
 } else {
 	url = "../" + url;	
 }
...
//AMV_DOCUMENTO_APPROVADataObjectHandler Tail @6-FCB6E20C
}
//End AMV_DOCUMENTO_APPROVADataObjectHandler Tail

...
//class DataObject Header @3-26DD9679
public class MenuDataObject extends DS {
//End class DataObject Header

//attributes of DataObject @3-E324EF4D
...
//properties of DataObject @3-C11CFC2B

    public void  setUrlID( long param ) {
        this.urlID.setValue( param );
    }

    public void  setUrlID( long param, Format ignore ) {
        this.urlID.setValue( param );
    }
...
//load @3-70FA8ED7
    boolean load() {
        ...
        command.setSql( "SELECT 'getLinkDoc('||link||')' doc from AMV_VISTA_DOCUMENTI "
                    + "where id_documento={ID}" );
        if ( urlID.getObjectValue() == null ) urlID.setValue( -1 );
        command.addParameter( "ID", urlID, null );
        command.setCountSql( "SELECT COUNT(*) FROM ( SELECT 'getLinkDoc('||link||')' doc from AMV_VISTA_DOCUMENTI where id_documento={ID} ) cnt " );
...
//loadDataBind @3-2E2D99BE
        if ( records == null || ! records.hasMoreElements() ) {
            ...
            while ( counter < rows.length && records.hasMoreElements() ) {
                ...
                row.setDOC_LINK(Utils.convertToString(ds.parse(record.get("DOC"), row.getDOC_LINKField())));
                rows[counter++] = row;
            }
        }
//End loadDataBind

//PARAMETRI DataSource @6-2FA99C10
...
//attributes of DataObject @6-DAAA07F2
...
//properties of DataObject @6-8DDBF2D9
...
//load @6-621FD942
    boolean load() {
        ...
        command.setSql( "select 'PAR1' nome_par, '' valore, 'Parametro 1' titolo,  "
                    + "{MVRIC} id_ric from dual" );
        if ( sesMVRIC.getObjectValue() == null ) sesMVRIC.setValue( 1 );
        command.addParameter( "MVRIC", sesMVRIC, null );
        command.setCountSql( "SELECT COUNT(*) FROM ( select 'PAR1' nome_par, '' valore, 'Parametro 1' titolo,  "
                                                         + "            {MVRIC} id_ric from dual ) cnt " );

//AmvGuidaAction imports @1-E85CDFFB
...
//End AmvGuidaAction imports

//AmvGuidaAction class @1-CAA36D61
public class AmvGuidaAction extends Action {

//End AmvGuidaAction class

//AmvGuidaAction: method perform @1-E4F0D8E5
    public String perform( HttpServletRequest req, HttpServletResponse resp, ServletContext context ) {
        ...
        page = new AmvGuidaModel( (CCSLocale) SessionStorage.getInstance( req ).getAttribute(Names.CCS_LOCALE_KEY));
        if (req.getAttribute(page.getName()+"Parent")==null) page.setCharacterEncoding("windows-1252");
        page.setResponse(resp);
        page.setRequest(req);
        res = ResourceBundle.getBundle("MessagesBundle", page.getCCSLocale().getLocale());
        req.setAttribute( "AmvGuidaModel", page );
...
//End AmvGuidaAction: method perform

//AmvGuidaAction: call children actions @1-21D6E008
        if (result == null) {
...
//End AmvGuidaAction: call children actions
...
//Guida Grid: set where parameters @2-B68D8D86
            ...
            ds.setSesMVSTILE( SessionStorage.getInstance(req).getAttribute("MVSTILE") );
...
//Guida Grid: method bind @2-669153A6
        public void bind(com.codecharge.components.Component model, GuidaRow[] rows ) {
            ...
            while ( counter < rows.length && rows[counter] != null ) {
                ...
                c = (com.codecharge.components.Control) hashRow.get("APERTURA");
                if ( c == null ) { 
                    c = (com.codecharge.components.Control) model.getControl("APERTURA").clone();
                    c.setValue(row.getAPERTURA());
                ...
                c = (com.codecharge.components.Control) hashRow.get("CHIUSURA");
                if ( c == null ) { 
                    c = (com.codecharge.components.Control) model.getControl("CHIUSURA").clone();
                    c.setValue(row.getCHIUSURA());
                    hashRow.put( c.getName(), c );
                }
...
//Guida Directory: getRowFieldByName @2-891DED98
        public Object getRowFieldByName( String name, GuidaRow row ) {
            Object value = null;
            if ( "APERTURA".equals(name) ) {
                value = row.getAPERTURA();
            } else if ( "GUIDA".equals(name) ) {
                value = row.getGUIDA();
            } else if ( "CHIUSURA".equals(name) ) {
                value = row.getCHIUSURA();
...
//GuidaPropria Grid: set where parameters @13-C2ABE2D2
            ds.setSesMVVC( SessionStorage.getInstance(req).getAttribute("MVVC") );
            ds.setSesMVID( SessionStorage.getInstance(req).getAttribute("MVID") );
            ds.setSesUtente( SessionStorage.getInstance(req).getAttribute("Utente") );
            ds.setSesIstanza( SessionStorage.getInstance(req).getAttribute("Istanza") );
            ds.setSesProgetto( SessionStorage.getInstance(req).getAttribute("Progetto") );
            ds.setSesMVID2( SessionStorage.getInstance(req).getAttribute("MVID2") );
            ds.setSesMVPC( SessionStorage.getInstance(req).getAttribute("MVPC") );
            ds.setSesMVCONTEXT( SessionStorage.getInstance(req).getAttribute("MVCONTEXT") );
            ds.setSesMVSTILE( SessionStorage.getInstance(req).getAttribute("MVSTILE") );
...
//GuidaPropria Grid: method bind @13-C6B7CCF3
...
//GuidaPropria Directory: getRowFieldByName @13-8DB3497E
...
//AmvGuida Page: method validate @1-104025BA
    boolean validate() {
        return true;
    }
//End AmvGuida Page: method validate

//AmvGuidaAction Tail @1-FCB6E20C
}
//End AmvGuidaAction Tail

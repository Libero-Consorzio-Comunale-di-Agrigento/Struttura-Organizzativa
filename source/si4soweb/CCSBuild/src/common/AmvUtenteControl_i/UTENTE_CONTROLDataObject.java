//UTENTE_CONTROL DataSource @2-3E2FE4EC
...
//load @2-2A473F63
    boolean load() {
        ...
        command.setSql( "select decode(AMV_UTENTE.IS_UTENTE_VALIDO(nvl('{IDUTE}','{MVUTE}'),'{Utente}','{Progetto}','{GroupID}'),1,nvl('{IDUTE}','{MVUTE}'),'') utente from dual" );
        ...
        command.setCountSql( "SELECT COUNT(*) FROM ( select decode(AMV_UTENTE.IS_UTENTE_VALIDO(nvl('{IDUTE}','{MVUTE}'),'{Utente}','{Progetto}','{GroupID}'),1,nvl('{IDUTE}','{MVUTE}'),'') utente from dual ) cnt " );
